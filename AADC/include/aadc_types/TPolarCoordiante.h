...
class TPolarCoordiante: public AADC_Type
{
...
public:
    ...
    TPolarCoordiante()
	{
        type_name = cString("tPolarCoordiante");
        ...
        {
            // get value
            if(IS_FAILED(adtf_ddl::access_element::find_index(factory, cString("f32Angle"), index.f32Angle)))
            {
                LOG_WARNING(cString::Format("COULD NOT FIND INDEX: %s.f32Angle", type_name));
            }

            // get timestamp
            if(IS_FAILED(adtf_ddl::access_element::find_index(factory, cString("f32Radius"),
                                                                    index.f32Radius)))
            {
    ...
    tResult writePin(cPinWriter &output, void *content_ptr, tTimeStamp streamTime)
    {
        ...
        if (IS_OK(res = alloc_sample(sample, streamTime)))
        {
            ...
            {
                LOG_WARNING(cString::Format("COULD NOT SET ELEMENT VALUE: %s.f32Angle for index %f",type_name, index.f32Angle));
                RETURN_ERROR(res);
            }
            if(IS_FAILED(res = oCodec.SetElementValue(index.f32Radius, content.f32Radius)))
            {

                LOG_WARNING(cString::Format("COULD NOT SET ELEMENT VALUE: %s.f32Radius for index %f", type_name, index.f32Radius));
                RETURN_ERROR(res);
            }
        }
        else
