...
bool LoadXMLData(const char *path, void *propertyStruct){
	// lucifer, king of hell has written this error prune shit
	// have fun with this, filthy humans!
    ...
    const char *xmlElementName = "Property";
    const char *xmlChildName = "Value";

    char* pStruct = (char *) propertyStruct;

    XMLElement * pProperty = pRoot->FirstChildElement(xmlElementName);
    while(pProperty != NULL){
        
        const char *type = pProperty->Attribute("type");
        XMLElement * pValue = pProperty->FirstChildElement(xmlChildName);
//TODO: template function instead of code copy paste (with function ptr in arg)
        if(!strncmp("tUInt32", type, 7)){
		//	printf( "Property Type: tUInt32");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                *((uint32_t *) pStruct) = (uint32_t) strtoul(value.c_str(), NULL, 10); 
                pStruct += sizeof(uint32_t);
                pValue = pValue->NextSiblingElement(xmlChildName);

            }    
        }
        else if(!strncmp("tInt32", type, 6) || !strncmp("tInt", type, 4)){
		//	printf( "Property Type: tInt32");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                *((int32_t *) pStruct) = (int32_t) strtol(value.c_str(), NULL, 10); 
                pStruct += sizeof(int32_t);
                pValue = pValue->NextSiblingElement(xmlChildName);
            }    
        }
        else if(!strncmp("tUInt64", type, 7)){
		//	printf( "Property Type: tUInt64");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                *((uint64_t *) pStruct) = (uint64_t) strtoull(value.c_str(), NULL, 10); 
                pStruct += sizeof(uint64_t);
                pValue = pValue->NextSiblingElement(xmlChildName);

            }    
        }
        else if(!strncmp("tInt64", type, 6)){
		//	printf( "Property Type: tInt64");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                *((int64_t *) pStruct) = (int64_t) strtoll(value.c_str(), NULL, 10); 
                pStruct += sizeof(int64_t);
                pValue = pValue->NextSiblingElement(xmlChildName);
            }    
        }
        else if(!strncmp("tFloat32", type, 8)){
			//printf( "Property Type: tFloat32");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                std::replace(value.begin(), value.end(), '.', ',');
                *((float *) pStruct) = (float) strtof(value.c_str(), NULL); 
                pStruct += sizeof(float);
                pValue = pValue->NextSiblingElement(xmlChildName);
            }    
        }
        else if(!strncmp("tBool", type, 5)){
		//	printf( "Property Type: tBool");
            while(pValue != NULL){

                std::string value = pValue->GetText();
                std::replace(value.begin(), value.end(), '.', ',');
                *((bool *) pStruct) = (bool) strtol(value.c_str(), NULL, 10); 
                pStruct += sizeof(bool);
                pValue = pValue->NextSiblingElement(xmlChildName);
            }    
        }
        else if(!strncmp("string", type, 6)){
		//	printf( "Property Type: string");
            while(pValue != NULL){
                std::string value = pValue->GetText();
                value = std::regex_replace(value, std::regex("^ +| +$|( ) +"), "$1");
                *((string *) pStruct) = value;
                pStruct += sizeof(string);
                pValue = pValue->NextSiblingElement(xmlChildName);
            }    
        } else {
        ...
        pProperty = pProperty->NextSiblingElement(xmlElementName);
    }
    return true;
}
