...
class TSignalValue: public AADC_Type 
{
...
public:
    ...
    TSignalValue()
	{
        type_name = cString("tSignalValue");
        ...
        {
            // get value
            if(IS_FAILED(adtf_ddl::access_element::find_index(factory, cString("f32Value"), index.f32Value)))
            {
    ...
    tResult readPin(cPinReader &input, void *content_ptr, tTimeStamp lastTimestamp){
        Data &content = *((Data *) content_ptr);
        ...
        if (IS_OK(res = input.GetLastSample(sample)))
        {
            ...
            if(IS_FAILED(res = oDecoder.GetElementValue(index.f32Value, &(content.f32Value))))
            {
                LOG_WARNING(cString::Format("COULD NOT GET ELEMENT VALUE: %s.f32Value for index %d", type_name, index.f32Value));
                RETURN_ERROR(res);
            }
	...
	std::string to_string(void *content_ptr){
        Data &content = *((Data *) content_ptr);
		
		std::string str = "ts:\t " + std::to_string(content.ui32ArduinoTimestamp);
		str += "\nval:\t " + std::to_string(content.f32Value);
		return str;
	}
